...
function timeSpanToString(startDate, endDate) {
  // console.log(startDate, endDate, new Date(endDate - startDate).toUTCString());
  const millisec = String(endDate - startDate).slice(-3);
  const temp = new Date(endDate - startDate).toUTCString().slice(17, 25);
  const res = `${temp}.${millisec}`;
  return res;
}
...
function angleBetweenClockHands(date) {
  ...
  let angle = Math.abs(hour * 60 - min * 11);
  if (angle > 360) {
    angle -= 360;
  }
